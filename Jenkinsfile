...
pipeline{
    ...
    stages {
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit cleuser/reddit:latest "
                       sh "docker push cleuser/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image cleuser/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.143.236.133 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://3.140.195.255:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#cw2-devsecops-cicd-alert', //update and provide your channel name
